...
;(async () => {
  const sender = await getMsgSender<IM>('contentJs')
  console.info('dlog-contentjs:7', 'send')
  const a = await sender.sendEventToBackgroundJS('ppp', { x: 'se' })
  console.info('dlog-contentjs:9', 'get', a)
